...
class BotRepository:
    ...
    async def trigger_mailing_test(self, chat_id: int, text: str, buttons: list[dict] | None = None):
        async with aiohttp.ClientSession(base_url=self.BOT_API_URL) as session:
            resp = await session.post(f"/api/mailing/test", json={"chat_id": chat_id, "text": text, "buttons": buttons})
            assert resp.status == 200, await resp.text()
